...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __call__(self, instance):
        ...
        truncate_tokens_pair(tokens_a, tokens_b, self.len_vis_input + self.max_len_b, max_len_b=self.max_len_b, trunc_seg=self.trunc_seg, always_truncate_tail=self.always_truncate_tail)
        ...
        if self.bar:
            ...
            if self.mode == "s2s":
                ...
                input_mask = torch.tensor([1] * len(tokens) + [0] * n_pad, dtype=torch.long).unsqueeze(0).expand(self.max_len, self.max_len).clone()
