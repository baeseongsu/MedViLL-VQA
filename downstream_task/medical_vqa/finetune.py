...
def main():
    ...
    parser.add_argument("--mask_prob", default=0.15, type=float, help="Number of prediction is sometimes less than max_pred when sequence is short.")

    parser.add_argument("--max_pred", type=int, default=3, help="Max tokens of prediction.")
    parser.add_argument(
        "--s2s_prob", default=1, type=float, help="Percentage of examples that are bi-uni-directional LM (seq2seq). This must be turned off!!!!!!! because this is not for seq2seq model!!!"
    )
    parser.add_argument("--bi_prob", default=0, type=float, help="Percentage of examples that are bidirectional LM.")
    ...
    parser.add_argument(
        "--output_dir", default="/home/edlab/ssbae/mimic_mv_real/mimic-cxr/downstream_model/", type=str, help="The output directory where the model predictions and checkpoints will be written."
    )
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=args.train_batch_size, sampler=train_sampler, num_workers=args.num_workers, collate_fn=batch_list_to_batch_tensors, pin_memory=True
    )
    ...
    if args.vqa_eval:
        ...
        if args.do_train:
            ...
            for i_epoch in trange(start_epoch, args.num_train_epochs + 1, desc="Epoch"):
                ...
                # if args.tasks == "vqa":
                print({"epoch total_acc": total_vqa_acc.item() / len(train_loss), "epoch closed_acc": total_closed_acc / num_close, "epoch open_acc": total_open_acc / num_open})
...
def vqa_eval(args, device, logger, bi_uni_pipeline, tokenizer, model, results_dict):
    test_dataset = data_loader.Img2txtDataset(
        args,
        "test",
        args.src_file,
        args.image_root,
    ...
    test_dataloader = DataLoader(
        test_dataset, batch_size=args.train_batch_size, sampler=test_sampler, num_workers=args.num_workers, shuffle=False, collate_fn=batch_list_to_batch_tensors, drop_last=True
    )
